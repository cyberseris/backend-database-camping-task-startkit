...
-- 5-5. 修改：`王小明` 現在已經加入直播室了，請在`COURSE_BOOKING`更新該筆預約資料（請注意，不要更新到已經取消的紀錄）：
    ...
    -- 2. 狀態`status` 設定為上課中
    UPDATE "COURSE_BOOKING" SET status='上課中', join_at='2024-11-25 14:01:59'
    WHERE user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') AND
    course_id = (SELECT id FROM "COURSE" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io')) AND
    status <> '課程已取消';

-- 5-6. 查詢：計算用戶王小明的購買堂數，顯示須包含以下欄位： user_id , total。 (需使用到 SUM 函式與 Group By)
    SELECT user_id, SUM(purchased_credits) AS total FROM "CREDIT_PURCHASE" WHERE 
    user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') GROUP BY user_id;

-- 5-7. 查詢：計算用戶王小明的已使用堂數，顯示須包含以下欄位： user_id , total。 (需使用到 Count 函式與 Group By)
    SELECT user_id, COUNT(user_id) AS total FROM "COURSE_BOOKING" WHERE 
    user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') AND status <> '課程已取消' GROUP BY user_id;
...
-- 5-8. [挑戰題] 查詢：請在一次查詢中，計算用戶王小明的剩餘可用堂數，顯示須包含以下欄位： user_id , remaining_credit
    ...
    from (SELECT user_id, SUM(purchased_credits) AS total_credit FROM "CREDIT_PURCHASE" WHERE 
    user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') GROUP BY user_id) as "CREDIT_PURCHASE"
    inner join (SELECT user_id, COUNT(user_id) AS used_credit FROM "COURSE_BOOKING" WHERE 
    user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') AND status <> '課程已取消' GROUP BY user_id) as "COURSE_BOOKING"
    on "COURSE_BOOKING".user_id = "CREDIT_PURCHASE".user_id;
...
-- 顯示須包含以下欄位： 組合包方案名稱, 銷售數量
    SELECT 
        ...
        COUNT(purchased_credits) AS "銷售數量"
    FROM "CREDIT_PURCHASE"
    INNER JOIN "CREDIT_PACKAGE" on "CREDIT_PACKAGE".id = credit_package_id
    WHERE "CREDIT_PURCHASE".purchase_at >='2024-12-01 00:00:00' AND
    "CREDIT_PURCHASE".purchase_at <= '2024-12-31 23:59:59' GROUP BY "CREDIT_PACKAGE".name

-- 6-4. 查詢：計算 12 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收
    SELECT SUM(price_paid) AS "總營收" FROM "CREDIT_PURCHASE"
    WHERE "CREDIT_PURCHASE".purchase_at >='2024-12-01 00:00:00' AND
    "CREDIT_PURCHASE".purchase_at <= '2024-12-31 23:59:59'; 
...
-- 顯示須包含以下欄位： 預約會員人數
    ...
    WHERE created_at >='2024-12-01 00:00:00' AND
    created_at <= '2024-12-31 23:59:59' AND status <> '課程已取消'
